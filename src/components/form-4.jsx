...
export default function Form4({summary, back, nameTrue, emailTrue, phoneTrue, price, showMessage, setShowMessage, theme}){
    ...
    return (
        <div className="form-4">
            ...
            <h4>Plan: <span style={{opacity:0.7, color: spanColorPlan}}>{plan ? plan : "'il n'y a pas de plan'"}</span></h4>
            <h4>Add-ons: <span style={{opacity:0.7, color: spanColorAddOns}}>{addons ? addons : "'il n'y a pas des add-ons'"}</span></h4>
            {name && email && plan ? <h5>Price : ${price}</h5> : <h5>Price : 'none'</h5>} 

            <div className="form-3-buttons"> 
            <button className={theme ? "form-4-back-dark" : "form-3-back"} onClick={back}>Go Back</button>
